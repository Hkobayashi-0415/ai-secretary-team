...
**補足事項**:
- Alembicは識別子を32文字で切り詰めるため、制約名・インデックス名は32文字以内に収める。
- テストデータベースでは `pgvector` 拡張を有効化する必要がある（例: `CREATE EXTENSION vector;` または拡張をバンドルしたイメージを使用）。
...
| バージョン | 日付 | 変更内容 |
|-----------|------|---------|
| v1.1 | 2025-09-15 | Alembic識別子長制限とpgvector拡張の有効化について追記 |
