...
export default function AssistantsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        res = await fetch(`${BASE}/assistants/${id}/`, { method: "DELETE" });
      }
      if (!res.ok) {
        const text = await res.text();
        throw new Error(`${res.status}: ${text}`);
  ...
  async function handleSave(id: string) {
    ...
    try {
      ...
      setList((prev) => prev.map((a) => (a.id === id ? { ...a, ...updated } as Assistant : a)));
      setEditingId(null);
    } catch (e: any) {
      setError(e.message ?? String(e));
    } finally {
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {isEditing ? (
                    <>
                      ...
                      <button data-testid="assistant-cancel" onClick={cancelEdit} style={{ padding: ".25rem .75rem" }}>
                        Cancel
                      </button>
                    </>
                  ) : (
                    <>
                      <button data-testid="assistant-edit" onClick={() => startEdit(a)} style={{ padding: ".25rem .75rem" }}>
                        Edit
                      </button>
                      <button
