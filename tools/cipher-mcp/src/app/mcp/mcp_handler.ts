...
async function handleAskCipherTool(agent: MemAgent, args: any): Promise<any> {
	...
	try {
		...
		if (backgroundOperations) {
			backgroundOperations.catch((backgroundError) => {
				logger.error('[MCP Handler] Background operation failed', {
					sessionId: session_id,
		...
		logger.info('[MCP Handler] ask_cipher tool completed successfully', {
			sessionId: session_id,
		...
		const errorMessage = error instanceof Error ? error.message : String(error);
		logger.error('[MCP Handler] Error in ask_cipher tool', { error: errorMessage, sessionId: session_id });
		throw new Error(`ask_cipher tool failed: ${errorMessage}`);
	}
}
