...
// This ensures MCP_SERVER_MODE and other settings are available
config();
...
export const env: EnvSchema = new Proxy({} as EnvSchema, {
	get(target, prop: string): any {
		switch (prop) {
			...
			// MCP Server Configuration
			case 'MCP_SERVER_MODE':
				return process.env.MCP_SERVER_MODE;
			case 'AGGREGATOR_CONFLICT_RESOLUTION':
				return process.env.AGGREGATOR_CONFLICT_RESOLUTION;
			case 'AGGREGATOR_TIMEOUT':
				return process.env.AGGREGATOR_TIMEOUT
					? parseInt(process.env.AGGREGATOR_TIMEOUT, 10)
					: undefined;
			default:
